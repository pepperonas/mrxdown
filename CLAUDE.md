...
- `--to pdf|html|docx|slides|epub` + files/dirs → `src/main/cli.js` `runCli()` converts everything in one loop (multiple args = shell globs; dirs expand to their .md files). `--pdf` is an alias for `--to pdf`.
...
- **Table editor (E3)**: pure table logic (parse/format/ops/CSV, `\|`-aware) in editor-utils.js; renderer glue in 10-dialogs.js (`getTableContext`/`applyTableOp` + `#tableToolbar`, shown via `updateTableToolbar()` hooked into `updateCursorPosition`). CSV/TSV paste sits in the K6 paste pipeline AFTER html conversion; comma needs ≥3 rows or ≥3 cols (prose guard).
...
- **AI assist (I1)**: opt-in only (`settings.ai.enabled`, never default-on). ALL requests run in the main process (`src/main/ai.js` — the renderer CSP has connect-src 'none'); the action/prompt catalog is closed and main-side; the API key is safeStorage-encrypted in `settings.aiApiKeyEnc`, write-only via `set-ai-api-key` (stripped from `save-settings` payloads, never sent to the renderer, never sent to Ollama). Streaming via `ai-chunk`/`ai-done`/`ai-error` events with requestId correlation; renderer UI in 12-features.js (`#aiModal`).
...
- `epub-export.test.js` — chapter split + EPUB structure (strict XML parse, mimetype spec) (K5)
- `pandoc.test.js` — pandoc detection + conversions (auto-skips without pandoc) (K3)
- `ai.test.js` — AI request builders per provider + stream parsers (no network) (I1)
- `slash-commands.test.js` — snippet placeholder expansion + built-in command catalog (E2)
- `table-editor.test.js` — table parse/format/ops + CSV/TSV detection heuristics (E3)
